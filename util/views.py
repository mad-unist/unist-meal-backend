...
class ProcessExcelMenuDormitory(APIView):
    ...
    def process_menu(self, worksheet, place):
        ...
        for i in range(7):  
            ...
            #     datas.append(make_menu(worksheet, place, day_idx[i], dates[i],week_lunch_idx[0], week_lunch_idx[1], week_lunch_idx[2])) # 점심 (한식)
            # else:
...
class ProcessExcelMenuStudent(APIView):
    ...
    def process_menu(self, worksheet, place):
        day_idx = ["E", "G", "I", "K", "M"]
        date_idx = 5
        week_lunch_idx = [6, 13, "점심"]
        evening_idx = [17, 24, "저녁"]
